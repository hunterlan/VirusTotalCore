...
internal interface IVirusTotalTransport
{
    ...
    Task PostAsync<TRequest>(string endpointName, string requestUrl, TRequest value, CancellationToken cancellationToken);
    Task<T> PostAsync<T, TRequest>(string endpointName, string requestUrl, string rootPropertyName, TRequest value, CancellationToken cancellationToken);
